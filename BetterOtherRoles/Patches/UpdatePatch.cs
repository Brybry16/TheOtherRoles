...
namespace BetterOtherRoles.Patches {
    ...
    class HudManagerUpdatePatch
    {
        ...
        static void setNameColors() {
            ...
            if (BetterOtherRoles.Deputy.deputy != null && BetterOtherRoles.Deputy.deputy == localPlayer) {
                setPlayerNameColor(BetterOtherRoles.Deputy.deputy, BetterOtherRoles.Deputy.color);
                if (BetterOtherRoles.Sheriff.sheriff != null && BetterOtherRoles.Deputy.knowsSheriff) {
                    setPlayerNameColor(BetterOtherRoles.Sheriff.sheriff, BetterOtherRoles.Sheriff.color);
                }
        ...
        static void setNameTags() {
            ...
            if (CachedPlayer.LocalPlayer != null && CachedPlayer.LocalPlayer.Data.Role.IsImpostor) {
                foreach (PlayerControl player in CachedPlayer.AllPlayers)
                    if (BetterOtherRoles.Godfather.godfather != null && BetterOtherRoles.Godfather.godfather == player)
                            player.cosmetics.nameText.text = player.Data.PlayerName + " (G)";
                    else if (BetterOtherRoles.Mafioso.mafioso != null && BetterOtherRoles.Mafioso.mafioso == player)
                            player.cosmetics.nameText.text = player.Data.PlayerName + " (M)";
                    else if (BetterOtherRoles.Janitor.janitor != null && BetterOtherRoles.Janitor.janitor == player)
                            player.cosmetics.nameText.text = player.Data.PlayerName + " (J)";
                if (MeetingHud.Instance != null)
                    foreach (PlayerVoteArea player in MeetingHud.Instance.playerStates)
                        if (BetterOtherRoles.Godfather.godfather != null && BetterOtherRoles.Godfather.godfather.PlayerId == player.TargetPlayerId)
                            player.NameText.text = BetterOtherRoles.Godfather.godfather.Data.PlayerName + " (G)";
                        else if (BetterOtherRoles.Mafioso.mafioso != null && BetterOtherRoles.Mafioso.mafioso.PlayerId == player.TargetPlayerId)
                            player.NameText.text = BetterOtherRoles.Mafioso.mafioso.Data.PlayerName + " (M)";
                        else if (BetterOtherRoles.Janitor.janitor != null && BetterOtherRoles.Janitor.janitor.PlayerId == player.TargetPlayerId)
                            player.NameText.text = BetterOtherRoles.Janitor.janitor.Data.PlayerName + " (J)";
            }
        ...
        static void timerUpdate() {
            ...
            HideNSeek.timer -= dt;
            foreach (byte key in BetterOtherRoles.Deputy.handcuffedKnows.Keys)
                BetterOtherRoles.Deputy.handcuffedKnows[key] -= dt;
        }
        ...
        static void updateImpostorKillButton(HudManager __instance) {
            ...
            if (Vampire.vampire != null && Vampire.vampire == CachedPlayer.LocalPlayer.PlayerControl)
                enabled = false;
            else if (BetterOtherRoles.Mafioso.mafioso != null && BetterOtherRoles.Mafioso.mafioso == CachedPlayer.LocalPlayer.PlayerControl && BetterOtherRoles.Godfather.godfather != null && !BetterOtherRoles.Godfather.godfather.Data.IsDead)
                enabled = false;
            else if (BetterOtherRoles.Janitor.janitor != null && BetterOtherRoles.Janitor.janitor == CachedPlayer.LocalPlayer.PlayerControl)
                enabled = false;
            
            if (enabled) __instance.KillButton.Show();
            else __instance.KillButton.Hide();

            if (BetterOtherRoles.Deputy.handcuffedKnows.ContainsKey(CachedPlayer.LocalPlayer.PlayerId) && BetterOtherRoles.Deputy.handcuffedKnows[CachedPlayer.LocalPlayer.PlayerId] > 0) __instance.KillButton.Hide();
        }

        static void updateReportButton(HudManager __instance) {
            if (GameOptionsManager.Instance.currentGameOptions.GameMode == GameModes.HideNSeek) return;
            if (BetterOtherRoles.Deputy.handcuffedKnows.ContainsKey(CachedPlayer.LocalPlayer.PlayerId) && BetterOtherRoles.Deputy.handcuffedKnows[CachedPlayer.LocalPlayer.PlayerId] > 0 || MeetingHud.Instance) __instance.ReportButton.Hide();
        ...
        {
            if (GameOptionsManager.Instance.currentGameOptions.GameMode == GameModes.HideNSeek) return;
            if (BetterOtherRoles.Deputy.handcuffedKnows.ContainsKey(CachedPlayer.LocalPlayer.PlayerId) && BetterOtherRoles.Deputy.handcuffedKnows[CachedPlayer.LocalPlayer.PlayerId] > 0 || MeetingHud.Instance) __instance.ImpostorVentButton.Hide();

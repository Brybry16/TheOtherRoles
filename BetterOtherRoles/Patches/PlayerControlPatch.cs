...
namespace BetterOtherRoles.Patches {
    ...
    public static class PlayerControlFixedUpdatePatch {
        ...
        static void sheriffSetTarget() {
            if (BetterOtherRoles.Sheriff.sheriff == null || BetterOtherRoles.Sheriff.sheriff != CachedPlayer.LocalPlayer.PlayerControl) return;
            BetterOtherRoles.Sheriff.currentTarget = setTarget();
            setPlayerOutline(BetterOtherRoles.Sheriff.currentTarget, BetterOtherRoles.Sheriff.color);
        }

        static void deputySetTarget()
        {
            if (BetterOtherRoles.Deputy.deputy == null || BetterOtherRoles.Deputy.deputy != CachedPlayer.LocalPlayer.PlayerControl) return;
            BetterOtherRoles.Deputy.currentTarget = setTarget();
            setPlayerOutline(BetterOtherRoles.Deputy.currentTarget, BetterOtherRoles.Deputy.color);
        }
        ...
        public static void deputyCheckPromotion(bool isMeeting=false)
        {
            ...
            if (BetterOtherRoles.Deputy.deputy == null || BetterOtherRoles.Deputy.deputy != CachedPlayer.LocalPlayer.PlayerControl) return;
            if (BetterOtherRoles.Deputy.promotesToSheriff == 0 || BetterOtherRoles.Deputy.deputy.Data.IsDead == true || BetterOtherRoles.Deputy.promotesToSheriff == 2 && !isMeeting) return;
            if (BetterOtherRoles.Sheriff.sheriff == null || BetterOtherRoles.Sheriff.sheriff?.Data?.Disconnected == true || BetterOtherRoles.Sheriff.sheriff.Data.IsDead)
            {
        ...
        static void detectiveUpdateFootPrints() {
            if (BetterOtherRoles.Detective.detective == null || BetterOtherRoles.Detective.detective != CachedPlayer.LocalPlayer.PlayerControl) return;

            BetterOtherRoles.Detective.timer -= Time.fixedDeltaTime;
            if (BetterOtherRoles.Detective.timer <= 0f) {
                BetterOtherRoles.Detective.timer = BetterOtherRoles.Detective.footprintIntervall;
        ...
        {
            if (CachedPlayer.LocalPlayer.PlayerControl == null || !BetterOtherRoles.Deputy.handcuffedKnows.ContainsKey(CachedPlayer.LocalPlayer.PlayerId)) return;
            
            if (BetterOtherRoles.Deputy.handcuffedKnows[CachedPlayer.LocalPlayer.PlayerId] <= 0)
            {
                BetterOtherRoles.Deputy.handcuffedKnows.Remove(CachedPlayer.LocalPlayer.PlayerId);
                // Resets the buttons
                BetterOtherRoles.Deputy.setHandcuffedKnows(false);
        ...
        static void engineerUpdate() {
            bool jackalHighlight = BetterOtherRoles.Engineer.highlightForTeamJackal && (CachedPlayer.LocalPlayer.PlayerControl == Jackal.jackal || CachedPlayer.LocalPlayer.PlayerControl == Sidekick.sidekick);
            bool impostorHighlight = BetterOtherRoles.Engineer.highlightForImpostors && CachedPlayer.LocalPlayer.Data.Role.IsImpostor;
            if ((jackalHighlight || impostorHighlight) && MapUtilities.CachedShipStatus?.AllVents != null) {
                foreach (Vent vent in MapUtilities.CachedShipStatus.AllVents) {
                    try {
                        if (vent?.myRend?.material != null) {
                            if (BetterOtherRoles.Engineer.engineer != null && BetterOtherRoles.Engineer.engineer.inVent) {
                                vent.myRend.material.SetFloat("_Outline", 1f);
                                vent.myRend.material.SetColor("_OutlineColor", BetterOtherRoles.Engineer.color);
                            }
        ...
        public static void miniCooldownUpdate() {
            if (Mini.mini != null && CachedPlayer.LocalPlayer.PlayerControl == Mini.mini) {
                ...
                HudManagerStartPatch.sheriffKillButton.MaxTimer = BetterOtherRoles.Sheriff.cooldown * multiplier;
    ...
    class BodyReportPatch
    {
        static void Postfix(PlayerControl __instance, [HarmonyArgument(0)]GameData.PlayerInfo target)
        {
            ...
            bool isDetectiveReport = BetterOtherRoles.Detective.detective != null && BetterOtherRoles.Detective.detective == CachedPlayer.LocalPlayer.PlayerControl && __instance.PlayerId == BetterOtherRoles.Detective.detective.PlayerId;
            if (isMedicReport || isDetectiveReport)
            {
                ...
                if (deadPlayer != null && deadPlayer.killerIfExisting != null) {
                    ...
                    if (isMedicReport) {
                        ...
                        if (timeSinceDeath < BetterOtherRoles.Detective.reportNameDuration * 1000) {
                            msg =  $"Body Report: The killer appears to be {deadPlayer.killerIfExisting.Data.PlayerName}!";
                        } else if (timeSinceDeath < BetterOtherRoles.Detective.reportColorDuration * 1000) {
    ...
    public static class ExilePlayerPatch
    {
        public static void Postfix(PlayerControl __instance)
        {
            ...
            if (Lawyer.lawyer != null && __instance == Lawyer.target) {
                ...
                if (AmongUsClient.Instance.AmHost && ((Lawyer.target != BetterOtherRoles.Jester.jester && !Lawyer.isProsecutor) || Lawyer.targetWasGuessed)) {
                    MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(CachedPlayer.LocalPlayer.PlayerControl.NetId, (byte)CustomRPC.LawyerPromotesToPursuer, Hazel.SendOption.Reliable, -1);
                    AmongUsClient.Instance.FinishRpcImmediately(writer);
                    RPCProcedure.lawyerPromotesToPursuer();
                }
    ...
    public static class IsFlashlightEnabledPatch {
        public static bool Prefix(ref bool __result) {
            if (GameOptionsManager.Instance.currentGameOptions.GameMode == GameModes.HideNSeek)
                return true;
            __result = false;
            if (!CachedPlayer.LocalPlayer.Data.IsDead && BetterOtherRoles.Lighter.lighter != null && BetterOtherRoles.Lighter.lighter.PlayerId == CachedPlayer.LocalPlayer.PlayerId) {
                __result = true;
            }
    ...
    public static class AdjustLight {
        public static bool Prefix(PlayerControl __instance) {
            if (__instance == null || CachedPlayer.LocalPlayer == null || BetterOtherRoles.Lighter.lighter == null) return true;

            bool hasFlashlight = !CachedPlayer.LocalPlayer.Data.IsDead && BetterOtherRoles.Lighter.lighter.PlayerId == CachedPlayer.LocalPlayer.PlayerId;
            __instance.SetFlashlightInputMethod();
            __instance.lightSource.SetupLightingForGameplay(hasFlashlight, BetterOtherRoles.Lighter.flashlightWidth, __instance.TargetFlashlight.transform);

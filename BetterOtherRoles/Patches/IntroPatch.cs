    ...
    class IntroPatch {
        ...
        class SetUpRoleTextPatch {
            ...
            static public void SetRoleTexts(IntroCutscene __instance) {
                ...
                if (BetterOtherRoles.Deputy.knowsSheriff && BetterOtherRoles.Deputy.deputy != null && BetterOtherRoles.Sheriff.sheriff != null) {
                    if (infos.Any(info => info.roleId == RoleId.Sheriff))
                        __instance.RoleBlurbText.text += Helpers.cs(BetterOtherRoles.Sheriff.color, $"\nYour Deputy is {BetterOtherRoles.Deputy.deputy?.Data?.PlayerName ?? ""}");
                    else if (infos.Any(info => info.roleId == RoleId.Deputy))
                        __instance.RoleBlurbText.text += Helpers.cs(BetterOtherRoles.Sheriff.color, $"\nYour Sheriff is {BetterOtherRoles.Sheriff.sheriff?.Data?.PlayerName ?? ""}");
                }
            }

...
namespace BetterOtherRoles.Patches {
    ...
    class MeetingHudPatch {
            ...
            private static Dictionary<byte, int> CalculateVotes(MeetingHud __instance) {
                ...
                for (int i = 0; i < __instance.playerStates.Length; i++) {
                    ...
                    if (playerVoteArea.VotedFor != 252 && playerVoteArea.VotedFor != 255 && playerVoteArea.VotedFor != 254) {
                        ...
                        int currentVotes;
                        int additionalVotes = (BetterOtherRoles.Mayor.mayor != null && BetterOtherRoles.Mayor.mayor.PlayerId == playerVoteArea.TargetPlayerId && BetterOtherRoles.Mayor.voteTwice) ? 2 : 1; // Mayor vote
        ...
        class MeetingHudBloopAVoteIconPatch {
            public static bool Prefix(MeetingHud __instance, GameData.PlayerInfo voterPlayer, int index, Transform parent) {
                ...
                var showVoteColors = !GameManager.Instance.LogicOptions.GetAnonymousVotes() ||
                                      ...
                                      (BetterOtherRoles.Mayor.mayor != null && BetterOtherRoles.Mayor.mayor == CachedPlayer.LocalPlayer.PlayerControl && BetterOtherRoles.Mayor.canSeeVoteColors && TasksHandler.taskInfo(CachedPlayer.LocalPlayer.Data).Item1 >= BetterOtherRoles.Mayor.tasksNeededToSeeVoteColors);
                if (showVoteColors)
                {
            ...
            private static bool Prefix(MeetingHud __instance, Il2CppStructArray<MeetingHud.VoterState> states) {
                ...
                for (int i = 0; i < __instance.playerStates.Length; i++) {
                    ...
                    for (int j = 0; j < states.Length; j++) {
                        ...
                        if (BetterOtherRoles.Mayor.mayor != null && voterState.VoterId == (sbyte)BetterOtherRoles.Mayor.mayor.PlayerId && !mayorFirstVoteDisplayed && BetterOtherRoles.Mayor.voteTwice) {
        ...
        class MeetingHudVotingCompletedPatch {
            static void Postfix(MeetingHud __instance, [HarmonyArgument(0)]byte[] states, [HarmonyArgument(1)]GameData.PlayerInfo exiled, [HarmonyArgument(2)]bool tie)
            {
                ...
                if (exiled != null) {
                    ...
                    Pursuer.notAckedExiled = (Pursuer.pursuer != null && Pursuer.pursuer.PlayerId == exiled.PlayerId) || (Lawyer.lawyer != null && Lawyer.target != null && Lawyer.target.PlayerId == exiled.PlayerId && Lawyer.target != BetterOtherRoles.Jester.jester && !Lawyer.isProsecutor);
                }
        ...
        static void mayorToggleVoteTwice(MeetingHud __instance) {
            __instance.playerStates[0].Cancel();  // This will stop the underlying buttons of the template from showing up
            if (__instance.state == MeetingHud.VoteStates.Results || BetterOtherRoles.Mayor.mayor.Data.IsDead) return;
            if (BetterOtherRoles.Mayor.mayorChooseSingleVote == 1) { // Only accept changes until the mayor voted
                var mayorPVA = __instance.playerStates.FirstOrDefault(x => x.TargetPlayerId == BetterOtherRoles.Mayor.mayor.PlayerId);
            ...
            BetterOtherRoles.Mayor.voteTwice = !BetterOtherRoles.Mayor.voteTwice;
            ...
            writer.Write(BetterOtherRoles.Mayor.voteTwice);
            AmongUsClient.Instance.FinishRpcImmediately(writer);

            meetingExtraButtonLabel.text = Helpers.cs(BetterOtherRoles.Mayor.color, "Double Vote: " + (BetterOtherRoles.Mayor.voteTwice ? Helpers.cs(Color.green, "On ") : Helpers.cs(Color.red, "Off")));
        }
        ...
        static void populateButtonsPostfix(MeetingHud __instance) {
            ...
            bool addMayorButton = BetterOtherRoles.Mayor.mayor != null && CachedPlayer.LocalPlayer.PlayerControl == BetterOtherRoles.Mayor.mayor && !BetterOtherRoles.Mayor.mayor.Data.IsDead && BetterOtherRoles.Mayor.mayorChooseSingleVote > 0;
        ...
        class StartMeetingPatch {
            public static void Prefix(PlayerControl __instance, [HarmonyArgument(0)]GameData.PlayerInfo meetingTarget) {
                ...
                if (BetterOtherRoles.Portalmaker.portalmaker != null && (CachedPlayer.LocalPlayer.PlayerControl == BetterOtherRoles.Portalmaker.portalmaker || Helpers.shouldShowGhostInfo()) && !BetterOtherRoles.Portalmaker.portalmaker.Data.IsDead) {
                    if (Portal.teleportedPlayers.Count > 0) {
                        ...
                        foreach (var entry in Portal.teleportedPlayers) {
                            ...
                            msg += BetterOtherRoles.Portalmaker.logShowsTime ? $"{(int)timeBeforeMeeting}s ago: " : "";
                            msg = msg + $"{entry.name} used the teleporter\n";
                        }
                        FastDestroyableSingleton<HudManager>.Instance.Chat.AddChat(BetterOtherRoles.Portalmaker.portalmaker, $"{msg}");
                    }
                }

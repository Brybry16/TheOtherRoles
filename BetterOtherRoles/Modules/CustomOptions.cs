...
namespace BetterOtherRoles {
    ...
    class GameOptionsMenuStartPatch {
        ...
        private static void createClassicTabs(GameOptionsMenu __instance) {
            ...
            var torTabHighlight = getTabHighlight(torTab, "TheOtherRolesTab", "BetterOtherRoles.Resources.TabIcon.png");
        ...
        private static void createHideNSeekTabs(GameOptionsMenu __instance) {
            ...
            for (int i = 0; i < CustomOption.options.Count; i++) {
                CustomOption option = CustomOption.options[i];
                if (option.type != CustomOption.CustomOptionType.HideNSeekMain && option.type != CustomOption.CustomOptionType.HideNSeekRoles) continue;
                if (option.optionBehaviour == null) {
        ...
        private static void createPropHuntTabs(GameOptionsMenu __instance) {
            ...
            for (int i = 0; i < CustomOption.options.Count; i++) {
                CustomOption option = CustomOption.options[i];
                if (option.type != CustomOption.CustomOptionType.PropHunt) continue;
                if (option.optionBehaviour == null) {
                    int index = 0;
                    StringOption stringOption = UnityEngine.Object.Instantiate(template, menus[index]);
                    optionBehaviours[index].Add(stringOption);
    ...
    class GameOptionsMenuUpdatePatch
    {
        ...
        public static void Postfix(GameOptionsMenu __instance) {
            ...
            float offset = 2.75f;
            foreach (CustomOption option in CustomOption.options) {
                if (GameObject.Find("TORSettings") && option.type != CustomOption.CustomOptionType.General && option.type != CustomOption.CustomOptionType.HideNSeekMain && option.type != CustomOption.CustomOptionType.PropHunt)
                    continue;
            ...
            foreach (var option in options) {
                if (option.parent == null) {
                    ...
                    if (option.id == 103) //Deputy
                        sb.AppendLine($"- {Helpers.cs(BetterOtherRoles.Deputy.color, "Deputy")}: {option.selections[option.selection].ToString()}");

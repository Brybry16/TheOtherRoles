...
namespace BetterOtherRoles
{
    ...
    static class HudManagerStartPatch
    {
        ...
        public static void setCustomButtonCooldowns() {
            ...
            janitorCleanButton.MaxTimer = BetterOtherRoles.Janitor.cooldown;
            sheriffKillButton.MaxTimer = BetterOtherRoles.Sheriff.cooldown;
            deputyHandcuffButton.MaxTimer = BetterOtherRoles.Deputy.handcuffCooldown;
            timeMasterShieldButton.MaxTimer = TimeMaster.cooldown;
            medicShieldButton.MaxTimer = 0f;
            shifterShiftButton.MaxTimer = 0f;
            morphlingButton.MaxTimer = Morphling.cooldown;
            camouflagerButton.MaxTimer = Camouflager.cooldown;
            portalmakerPlacePortalButton.MaxTimer = BetterOtherRoles.Portalmaker.cooldown;
            usePortalButton.MaxTimer = BetterOtherRoles.Portalmaker.usePortalCooldown;
            portalmakerMoveToPortalButton.MaxTimer = BetterOtherRoles.Portalmaker.usePortalCooldown;
        ...
        private static void addReplacementHandcuffedButton(CustomButton button, Vector3? positionOffset = null, Func<bool> couldUse = null)
        {
            ...
            CustomButton replacementHandcuffedButton = new CustomButton(() => { }, () => { return true; }, couldUse, () => { }, BetterOtherRoles.Deputy.getHandcuffedButtonSprite(), positionOffsetValue, button.hudManager, button.hotkey,
                true, BetterOtherRoles.Deputy.handcuffDuration, () => { }, button.mirror);
        ...
        public static void createButtonsPostfix(HudManager __instance) {
            ...
            engineerRepairButton = new CustomButton(
                ...
                () => { return BetterOtherRoles.Engineer.engineer != null && BetterOtherRoles.Engineer.engineer == CachedPlayer.LocalPlayer.PlayerControl && BetterOtherRoles.Engineer.remainingFixes > 0 && !CachedPlayer.LocalPlayer.Data.IsDead; },
                () => {
                    ...
                    foreach (PlayerTask task in CachedPlayer.LocalPlayer.PlayerControl.myTasks.GetFastEnumerator())
                        if (task.TaskType == TaskTypes.FixLights || task.TaskType == TaskTypes.RestoreOxy || task.TaskType == TaskTypes.ResetReactor || task.TaskType == TaskTypes.ResetSeismic || task.TaskType == TaskTypes.FixComms || task.TaskType == TaskTypes.StopCharles
                            ...
                            sabotageActive = true;
                    return sabotageActive && BetterOtherRoles.Engineer.remainingFixes > 0 && CachedPlayer.LocalPlayer.PlayerControl.CanMove;
                },
                () => {},
                BetterOtherRoles.Engineer.getButtonSprite(),
            ...
            janitorCleanButton = new CustomButton(
                        ...
                        if (collider2D.tag == "DeadBody")
                        {
                            ...
                            if (component && !component.Reported)
                            {
                                ...
                                if (Vector2.Distance(truePosition2, truePosition) <= CachedPlayer.LocalPlayer.PlayerControl.MaxReportDistance && CachedPlayer.LocalPlayer.PlayerControl.CanMove && !PhysicsHelpers.AnythingBetween(truePosition, truePosition2, Constants.ShipAndObjectsMask, false))
                                {
                                    ...
                                    writer.Write(BetterOtherRoles.Janitor.janitor.PlayerId);
                                    AmongUsClient.Instance.FinishRpcImmediately(writer);
                                    RPCProcedure.cleanBody(playerInfo.PlayerId, BetterOtherRoles.Janitor.janitor.PlayerId);
                ...
                () => { return BetterOtherRoles.Janitor.janitor != null && BetterOtherRoles.Janitor.janitor == CachedPlayer.LocalPlayer.PlayerControl && !CachedPlayer.LocalPlayer.Data.IsDead; },
                () => { return __instance.ReportButton.graphic.color == Palette.EnabledColor && CachedPlayer.LocalPlayer.PlayerControl.CanMove; },
                () => { janitorCleanButton.Timer = janitorCleanButton.MaxTimer; },
                BetterOtherRoles.Janitor.getButtonSprite(),
            ...
            sheriffKillButton = new CustomButton(
                () => {
                    MurderAttemptResult murderAttemptResult = Helpers.checkMuderAttempt(BetterOtherRoles.Sheriff.sheriff, BetterOtherRoles.Sheriff.currentTarget);
                    if (murderAttemptResult == MurderAttemptResult.SuppressKill) return;

                    if (murderAttemptResult == MurderAttemptResult.PerformKill) {
                        byte targetId = 0;
                        if ((BetterOtherRoles.Sheriff.currentTarget.Data.Role.IsImpostor && (BetterOtherRoles.Sheriff.currentTarget != Mini.mini || Mini.isGrownUp())) ||
                            (BetterOtherRoles.Sheriff.spyCanDieToSheriff && Spy.spy == BetterOtherRoles.Sheriff.currentTarget) ||
                            (BetterOtherRoles.Sheriff.canKillNeutrals && Helpers.isNeutral(BetterOtherRoles.Sheriff.currentTarget)) ||
                            (Jackal.jackal == BetterOtherRoles.Sheriff.currentTarget || Sidekick.sidekick == BetterOtherRoles.Sheriff.currentTarget)) {
                            targetId = BetterOtherRoles.Sheriff.currentTarget.PlayerId;
                        }
                        else {
                        ...
                        killWriter.Write(BetterOtherRoles.Sheriff.sheriff.Data.PlayerId);
                        killWriter.Write(targetId);
                        killWriter.Write(byte.MaxValue);
                        AmongUsClient.Instance.FinishRpcImmediately(killWriter);
                        RPCProcedure.uncheckedMurderPlayer(BetterOtherRoles.Sheriff.sheriff.Data.PlayerId, targetId, Byte.MaxValue);
                    }

                    sheriffKillButton.Timer = sheriffKillButton.MaxTimer;
                    BetterOtherRoles.Sheriff.currentTarget = null;
                },
                () => { return BetterOtherRoles.Sheriff.sheriff != null && BetterOtherRoles.Sheriff.sheriff == CachedPlayer.LocalPlayer.PlayerControl && !CachedPlayer.LocalPlayer.Data.IsDead; },
                () => { return BetterOtherRoles.Sheriff.currentTarget && CachedPlayer.LocalPlayer.PlayerControl.CanMove; },
                () => { sheriffKillButton.Timer = sheriffKillButton.MaxTimer;},
            ...
            deputyHandcuffButton = new CustomButton(
                () => {
                    byte targetId = 0;
                    targetId = BetterOtherRoles.Sheriff.sheriff == CachedPlayer.LocalPlayer.PlayerControl ? BetterOtherRoles.Sheriff.currentTarget.PlayerId : BetterOtherRoles.Deputy.currentTarget.PlayerId;  // If the deputy is now the sheriff, sheriffs target, else deputies target
                    ...
                    BetterOtherRoles.Deputy.currentTarget = null;
                    deputyHandcuffButton.Timer = deputyHandcuffButton.MaxTimer;

                    SoundEffectsManager.play("deputyHandcuff");
                },
                () => { return (BetterOtherRoles.Deputy.deputy != null && BetterOtherRoles.Deputy.deputy == CachedPlayer.LocalPlayer.PlayerControl || BetterOtherRoles.Sheriff.sheriff != null && BetterOtherRoles.Sheriff.sheriff == CachedPlayer.LocalPlayer.PlayerControl && BetterOtherRoles.Sheriff.sheriff == BetterOtherRoles.Sheriff.formerDeputy && BetterOtherRoles.Deputy.keepsHandcuffsOnPromotion) && !CachedPlayer.LocalPlayer.Data.IsDead; },
                () => {
                    if (deputyButtonHandcuffsText != null) deputyButtonHandcuffsText.text = $"{BetterOtherRoles.Deputy.remainingHandcuffs}";
                    return ((BetterOtherRoles.Deputy.deputy != null && BetterOtherRoles.Deputy.deputy == CachedPlayer.LocalPlayer.PlayerControl && BetterOtherRoles.Deputy.currentTarget || BetterOtherRoles.Sheriff.sheriff != null && BetterOtherRoles.Sheriff.sheriff == CachedPlayer.LocalPlayer.PlayerControl && BetterOtherRoles.Sheriff.sheriff == BetterOtherRoles.Sheriff.formerDeputy && BetterOtherRoles.Sheriff.currentTarget) && BetterOtherRoles.Deputy.remainingHandcuffs > 0 && CachedPlayer.LocalPlayer.PlayerControl.CanMove);
                },
                () => { deputyHandcuffButton.Timer = deputyHandcuffButton.MaxTimer; },
                BetterOtherRoles.Deputy.getButtonSprite(),
            ...
            portalmakerPlacePortalButton = new CustomButton(
                ...
                () => { return BetterOtherRoles.Portalmaker.portalmaker != null && BetterOtherRoles.Portalmaker.portalmaker == CachedPlayer.LocalPlayer.PlayerControl && !CachedPlayer.LocalPlayer.Data.IsDead && Portal.secondPortal == null; },
                () => { return CachedPlayer.LocalPlayer.PlayerControl.CanMove && Portal.secondPortal == null; },
                () => { portalmakerPlacePortalButton.Timer = portalmakerPlacePortalButton.MaxTimer; },
                BetterOtherRoles.Portalmaker.getPlacePortalButtonSprite(),
                CustomButton.ButtonPositions.lowerRowRight,
                __instance,
                KeyCode.F
            );
            ...
            usePortalButton = new CustomButton(
                ...
                () => {
                    if (CachedPlayer.LocalPlayer.PlayerControl == BetterOtherRoles.Portalmaker.portalmaker && Portal.bothPlacedAndEnabled)
                        portalmakerButtonText1.text = Portal.locationNearEntry(CachedPlayer.LocalPlayer.transform.position) || !BetterOtherRoles.Portalmaker.canPortalFromAnywhere ? "" : "1. " + Portal.firstPortal.room;
                    return Portal.bothPlacedAndEnabled; },
                () => { return CachedPlayer.LocalPlayer.PlayerControl.CanMove && (Portal.locationNearEntry(CachedPlayer.LocalPlayer.transform.position) || BetterOtherRoles.Portalmaker.canPortalFromAnywhere && CachedPlayer.LocalPlayer.PlayerControl == BetterOtherRoles.Portalmaker.portalmaker) && !Portal.isTeleporting; },
                () => { usePortalButton.Timer = usePortalButton.MaxTimer; },
                BetterOtherRoles.Portalmaker.getUsePortalButtonSprite(),
            ...
            portalmakerMoveToPortalButton = new CustomButton(
                ...
                () => { return BetterOtherRoles.Portalmaker.canPortalFromAnywhere && Portal.bothPlacedAndEnabled && CachedPlayer.LocalPlayer.PlayerControl == BetterOtherRoles.Portalmaker.portalmaker; },
            ...
            mayorMeetingButton = new CustomButton(
               () => {
                   CachedPlayer.LocalPlayer.NetTransform.Halt(); // Stop current movement 
                   BetterOtherRoles.Mayor.remoteMeetingsLeft--;
               ...
               () => { return BetterOtherRoles.Mayor.mayor != null && BetterOtherRoles.Mayor.mayor == CachedPlayer.LocalPlayer.PlayerControl && !CachedPlayer.LocalPlayer.Data.IsDead && BetterOtherRoles.Mayor.meetingButton; },
               () => {
                   mayorMeetingButton.actionButton.OverrideText("Emergency ("+ BetterOtherRoles.Mayor.remoteMeetingsLeft + ")");
                   bool sabotageActive = false;
                   foreach (PlayerTask task in CachedPlayer.LocalPlayer.PlayerControl.myTasks.GetFastEnumerator())
                       if (task.TaskType == TaskTypes.FixLights || task.TaskType == TaskTypes.RestoreOxy || task.TaskType == TaskTypes.ResetReactor || task.TaskType == TaskTypes.ResetSeismic || task.TaskType == TaskTypes.FixComms || task.TaskType == TaskTypes.StopCharles
                           || SubmergedCompatibility.IsSubmerged && task.TaskType == SubmergedCompatibility.RetrieveOxygenMask)
                           sabotageActive = true;
                   return !sabotageActive && CachedPlayer.LocalPlayer.PlayerControl.CanMove && (BetterOtherRoles.Mayor.remoteMeetingsLeft > 0);
               },
               () => { mayorMeetingButton.Timer = mayorMeetingButton.MaxTimer; },
               BetterOtherRoles.Mayor.getMeetingSprite(),
               CustomButton.ButtonPositions.lowerRowRight,
               __instance,
               KeyCode.F,
               true,
